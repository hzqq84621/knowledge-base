...
`,
    ...
    knowledgeConfiguration: {
      ...
      manual: `<p><b>支持格式：</b>PDF</p>
      <p><b>适用场景：</b></p><p>
      适用于维修手册等pdf文档，按章节标题分割，同一章节内的<i>图文内容保持完整</i>。</p>`,

      naive: `<p><b>支持格式：</b>DOCX、XLSX、XLS (Excel97~2003)、PPT、PDF、TXT、JPEG、JPG、PNG、TIF、GIF、CSV、JSON、EML、HTML</p>
      <p><b>适用场景：</b></p>
      <li>此方法为通用处理方法。</li>
      ...
      <p><b>适用场景：</b></p><p>
      适用于工艺技术文档等资料，按<i>摘要、工艺章节</i>等结构分割，建议调整topN参数平衡效果与成本。</p>`,

      presentation: `<p><b>支持格式：</b>PDF、PPTX</p><p>
      <b>适用场景：</b>适用于演讲资料、培训资料等文档，自动按<i>页面分块</i>，存储页面缩略图便于预览。</p>`,

      qa: ` <p><b>支持格式：</b> excel 和 csv/txt </p>
     <p><b>适用场景：</b></p><p>
      <li>
      如果文件是<b> excel </b>格式，则应由两个列组成
      没有标题：一个提出问题，另一个用于答案。多张纸只要列结构正确，就可以接受。
    </li>
    <li>
      ...
      resume: `<p><b>支持格式：</b>DOCX、PDF、TXT</p>
      <p><b>适用场景：</b></p><p>
      <li>
      ...
      您只需与<i>'Agent'</i>交谈即可列出所有符合资格的候选人。
      </li>
      </p>
        `,
      table: `支持格式：<p><b>XLSX</b>和<b>CSV/TXT</b>。</p><p>
      <p><b>适用场景：</b></p><p>
      适用于生产报表等表格资料的切分</p>
      <ul>
      <p><b>注意事项：</p><p>
      ...
      picture: `
      <p><b>支持格式：</b>图像文件。视频即将推出。</p><p>
      如果图片中有文字，则应用 OCR 提取文字作为其文字描述。
      </p><p>
      如果OCR提取的文本不够，可以使用视觉LLM来获取描述。
      </p>`,
      one: `
      <p><b>支持格式：</b>DOCX、EXCEL、PDF、TXT</p>
      <p><b>适用场景：</b></p><p>
      如果你要总结的东西需要一篇文章的全部上下文，并且所选LLM的上下文长度覆盖了文档长度，你可以尝试这种方法。
      </p><p>
      <p><b>注意事项：</p><p>
      对于一个文档，它将被视为一个完整的块，根本不会被分割。</p>`,

      knowledgeGraph: `<p><b>支持格式：</b>DOCX、EXCEL、PPT、IMAGE、PDF、TXT、MD、JSON、EML</p>
      <p><b>适用场景：</b>文件分块后，使用分块提取整个文档的知识图谱和思维导图。此方法将简单的方法应用于分块文件：
      连续的文本将被切成大约 512 个 token 数的块。</p>
      <p>接下来，将分块传输到 LLM 以提取知识图谱和思维导图的节点和关系。</p>
      <p><b>注意事项：</b>您需要指定的条目类型。</p>`,

      tag: `<p><b>支持格式：</b>XLSX和CSV/TXT</p>
      <p><b>适用场景：</b>使用“Tag”分块方法的知识库用作标签集.其他知识库可以把标签集当中的标签按照相似度匹配到自己对应的文本块中，对这些知识库的查询也将根据此标签集对自己进行标记。</p>
      <p><b>注意事项：</p><p>
      使用“标签”作为分块方法的知识库<b>不</b>参与 RAG 检索过程。</p>
